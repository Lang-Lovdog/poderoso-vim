...
if os.getenv("ZELLIJ") == "0" then
  require('liotree.config').setup({
    executers = {
        ...
        ["%.qt.plt$"    ] = "zellij ac new-pane -f --cwd __path__ -x \"80\\%\" -y \"00\\%\" --height \"20\\%\" --width \"20\\%\" --pinned true -c -- bash -c \"gnuplot-qt -p    __this__    ; echo END OF PROCESS ; read \" && zellij ac toggle-floating-panes   ",
        ...
        ["%.wx.plt$"    ] = "zellij ac new-pane -f --cwd __path__ -x \"80\\%\" -y \"00\\%\" --height \"20\\%\" --width \"20\\%\" --pinned true -c -- bash -c \"gnuplot-wx -p    __this__    ; echo END OF PROCESS ; read \" && zellij ac toggle-floating-panes   ",
        ["%.tex.puml$"  ] = "zellij ac new-pane -f --cwd __path__ -x \"80\\%\" -y \"00\\%\" --height \"20\\%\" --width \"20\\%\" --pinned true -c -- bash -c \"plantuml -tlatex:nopreamble $(basename __this__)    ; echo END OF PROCESS ; read \" && zellij ac toggle-floating-panes   ",
        ["%.puml$"      ] = "zellij ac new-pane -f --cwd __path__ -x \"80\\%\" -y \"00\\%\" --height \"20\\%\" --width \"20\\%\" --pinned true -c -- bash -c \"plantuml                    $(basename __this__)    ; echo END OF PROCESS ; read \" && zellij ac toggle-floating-panes   ",
