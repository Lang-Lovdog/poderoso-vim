package io.github.treesitter.jtreesitter.liotree;

import java.lang.foreign.*;

public final class TreeSitterLiotree {
    ...
    private static final TreeSitterLiotree INSTANCE = new TreeSitterLiotree();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterLiotree() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_liotree");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-liotree");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-liotree");
